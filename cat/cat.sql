...
CREATE DEFINER=`root`@`%` PROCEDURE `qy_insert_g_ara_tms`(IN in_csv_id int, IN in_purchase_cost varchar(45), IN in_list_refresh_cost varchar(45))
BEGIN 
insert into g_ara_tms (`csv_id`, `purchase_cost`, `list_refresh_cost`) values (in_csv_id, in_purchase_cost, in_list_refresh_cost)
on duplicate key update `csv_id` = in_csv_id, `purchase_cost` = in_purchase_cost, `list_refresh_cost` = in_list_refresh_cost;
END$$ 
DELIMITER ;
DELIMITER $$
...
BEGIN 
insert into g_config (`csv_id`, `user_level_max`, `user_vip_max`, `xilian_begain_level`, `cp_chapter_max`, `purch_phy_power`, `diamond_per_sec`, `ara_clg_tms_rst_tp`, `worship_reward_id`, `worship_reward_num`, `ara_clg_tms_max`, `ara_clg_tms_rst`, `ara_integral_rst`, `ara_clg_tms_pur_tms_rst`) values (in_csv_id, in_user_level_max, in_user_vip_max, in_xilian_begain_level, in_cp_chapter_max, in_purch_phy_power, in_diamond_per_sec, in_ara_clg_tms_rst_tp, in_worship_reward_id, in_worship_reward_num, in_ara_clg_tms_max, in_ara_clg_tms_rst, in_ara_integral_rst, in_ara_clg_tms_pur_tms_rst)
...
DELIMITER $$
=======
>>>>>>> 22dfcf9ad796237021cd0f93acf2e24e5468ba2d
...
DELIMITER ;
DELIMITER $$
CREATE DEFINER=`root`@`%` PROCEDURE `qy_insert_public_email`(IN in_id int, IN in_csv_id int, IN in_type int, IN in_title varchar(32), IN in_content varchar(32), IN in_acctime int, IN in_deltime int, IN in_isread tinyint, IN in_isdel tinyint, IN in_itemsn1 int, IN in_itemnum1 int, IN in_itemsn2 int, IN in_itemnum2 int, IN in_itemsn3 int, IN in_itemnum3 int, IN in_itemsn4 int, IN in_itemnum4 int, IN in_itemsn5 int, IN in_itemnum5 int, IN in_iconid int, IN in_isreward tinyint)
BEGIN 
insert into public_email (`id`, `csv_id`, `type`, `title`, `content`, `acctime`, `deltime`, `isread`, `isdel`, `itemsn1`, `itemnum1`, `itemsn2`, `itemnum2`, `itemsn3`, `itemnum3`, `itemsn4`, `itemnum4`, `itemsn5`, `itemnum5`, `iconid`, `isreward`) values (in_id, in_csv_id, in_type, in_title, in_content, in_acctime, in_deltime, in_isread, in_isdel, in_itemsn1, in_itemnum1, in_itemsn2, in_itemnum2, in_itemsn3, in_itemnum3, in_itemsn4, in_itemnum4, in_itemsn5, in_itemnum5, in_iconid, in_isreward)
on duplicate key update `id` = in_id, `csv_id` = in_csv_id, `type` = in_type, `title` = in_title, `content` = in_content, `acctime` = in_acctime, `deltime` = in_deltime, `isread` = in_isread, `isdel` = in_isdel, `itemsn1` = in_itemsn1, `itemnum1` = in_itemnum1, `itemsn2` = in_itemsn2, `itemnum2` = in_itemnum2, `itemsn3` = in_itemsn3, `itemnum3` = in_itemnum3, `itemsn4` = in_itemsn4, `itemnum4` = in_itemnum4, `itemsn5` = in_itemsn5, `itemnum5` = in_itemnum5, `iconid` = in_iconid, `isreward` = in_isreward;
END$$ 
DELIMITER ;
DELIMITER $$
...
DELIMITER ;
DELIMITER $$
CREATE DEFINER=`root`@`%` PROCEDURE `qy_insert_u_ara_rnk_rwd`(IN in_id bigint, IN in_user_id int, IN in_csv_id int, IN in_is_collected tinyint)
BEGIN 
insert into u_ara_rnk_rwd (`id`, `user_id`, `csv_id`, `is_collected`) values (in_id, in_user_id, in_csv_id, in_is_collected)
on duplicate key update `id` = in_id, `user_id` = in_user_id, `csv_id` = in_csv_id, `is_collected` = in_is_collected;
END$$ 
DELIMITER ;
DELIMITER $$
CREATE DEFINER=`root`@`%` PROCEDURE `qy_insert_u_cgold`(IN in_id bigint, IN in_user_id int, IN in_cgold_time int, IN in_cgold_type int, IN in_time_length int)
BEGIN 
insert into u_cgold (`id`, `user_id`, `cgold_time`, `cgold_type`, `time_length`) values (in_id, in_user_id, in_cgold_time, in_cgold_type, in_time_length)
on duplicate key update `id` = in_id, `user_id` = in_user_id, `cgold_time` = in_cgold_time, `cgold_type` = in_cgold_type, `time_length` = in_time_length;
END$$ 
DELIMITER ;
DELIMITER $$
CREATE DEFINER=`root`@`%` PROCEDURE `qy_insert_u_checkin`(IN in_id bigint, IN in_csv_id int, IN in_user_id int, IN in_u_checkin_time int, IN in_ifcheck_in tinyint)
BEGIN 
insert into u_checkin (`id`, `csv_id`, `user_id`, `u_checkin_time`, `ifcheck_in`) values (in_id, in_csv_id, in_user_id, in_u_checkin_time, in_ifcheck_in)
on duplicate key update `id` = in_id, `csv_id` = in_csv_id, `user_id` = in_user_id, `u_checkin_time` = in_u_checkin_time, `ifcheck_in` = in_ifcheck_in;
END$$ 
DELIMITER ;
DELIMITER $$
...
DELIMITER ;
DELIMITER $$
CREATE DEFINER=`root`@`%` PROCEDURE `qy_insert_u_checkpoint_rc`(IN in_id bigint, IN in_user_id int, IN in_csv_id int, IN in_passed int, IN in_cd_starttime int, IN in_cd_finished int, IN in_cd_walk int, IN in_hanging_starttime int, IN in_hanging_walk int, IN in_hanging_drop_starttime int, IN in_hanging_drop_walk int)
BEGIN 
insert into u_checkpoint_rc (`id`, `user_id`, `csv_id`, `passed`, `cd_starttime`, `cd_finished`, `cd_walk`, `hanging_starttime`, `hanging_walk`, `hanging_drop_starttime`, `hanging_drop_walk`) values (in_id, in_user_id, in_csv_id, in_passed, in_cd_starttime, in_cd_finished, in_cd_walk, in_hanging_starttime, in_hanging_walk, in_hanging_drop_starttime, in_hanging_drop_walk)
on duplicate key update `id` = in_id, `user_id` = in_user_id, `csv_id` = in_csv_id, `passed` = in_passed, `cd_starttime` = in_cd_starttime, `cd_finished` = in_cd_finished, `cd_walk` = in_cd_walk, `hanging_starttime` = in_hanging_starttime, `hanging_walk` = in_hanging_walk, `hanging_drop_starttime` = in_hanging_drop_starttime, `hanging_drop_walk` = in_hanging_drop_walk;
END$$ 
DELIMITER ;
DELIMITER $$
CREATE DEFINER=`root`@`%` PROCEDURE `qy_insert_u_equipment`(IN in_id bigint, IN in_user_id int, IN in_csv_id int, IN in_level int, IN in_combat int, IN in_defense int, IN in_critical_hit int, IN in_king int, IN in_critical_hit_probability int, IN in_combat_probability int, IN in_defense_probability int, IN in_king_probability int, IN in_enhance_success_rate int, IN in_currency_type int, IN in_currency_num int)
BEGIN 
insert into u_equipment (`id`, `user_id`, `csv_id`, `level`, `combat`, `defense`, `critical_hit`, `king`, `critical_hit_probability`, `combat_probability`, `defense_probability`, `king_probability`, `enhance_success_rate`, `currency_type`, `currency_num`) values (in_id, in_user_id, in_csv_id, in_level, in_combat, in_defense, in_critical_hit, in_king, in_critical_hit_probability, in_combat_probability, in_defense_probability, in_king_probability, in_enhance_success_rate, in_currency_type, in_currency_num)
on duplicate key update `id` = in_id, `user_id` = in_user_id, `csv_id` = in_csv_id, `level` = in_level, `combat` = in_combat, `defense` = in_defense, `critical_hit` = in_critical_hit, `king` = in_king, `critical_hit_probability` = in_critical_hit_probability, `combat_probability` = in_combat_probability, `defense_probability` = in_defense_probability, `king_probability` = in_king_probability, `enhance_success_rate` = in_enhance_success_rate, `currency_type` = in_currency_type, `currency_num` = in_currency_num;
END$$ 
DELIMITER ;
DELIMITER $$
CREATE DEFINER=`root`@`%` PROCEDURE `qy_insert_u_exercise`(IN in_id bigint, IN in_user_id int, IN in_exercise_time int, IN in_exercise_type int, IN in_time_length int)
BEGIN 
insert into u_exercise (`id`, `user_id`, `exercise_time`, `exercise_type`, `time_length`) values (in_id, in_user_id, in_exercise_time, in_exercise_type, in_time_length)
on duplicate key update `id` = in_id, `user_id` = in_user_id, `exercise_time` = in_exercise_time, `exercise_type` = in_exercise_type, `time_length` = in_time_length;
END$$ 
DELIMITER ;
DELIMITER $$
...
DELIMITER ;
DELIMITER $$
CREATE DEFINER=`root`@`%` PROCEDURE `qy_insert_u_lilian_phy_power`(IN in_id int, IN in_user_id int, IN in_csv_id int, IN in_start_time int, IN in_end_time int, IN in_purch_time int, IN in_num int)
BEGIN 
...
DELIMITER ;
DELIMITER $$
CREATE DEFINER=`root`@`%` PROCEDURE `qy_insert_u_new_draw`(IN in_id int, IN in_uid int, IN in_drawtype int, IN in_srecvtime int, IN in_propid int, IN in_amount int, IN in_iffree tinyint)
BEGIN 
insert into u_new_draw (`id`, `uid`, `drawtype`, `srecvtime`, `propid`, `amount`, `iffree`) values (in_id, in_uid, in_drawtype, in_srecvtime, in_propid, in_amount, in_iffree)
on duplicate key update `id` = in_id, `uid` = in_uid, `drawtype` = in_drawtype, `srecvtime` = in_srecvtime, `propid` = in_propid, `amount` = in_amount, `iffree` = in_iffree;
END$$ 
DELIMITER ;
DELIMITER $$
CREATE DEFINER=`root`@`%` PROCEDURE `qy_insert_u_new_email`(IN in_id int, IN in_csv_id int, IN in_uid int, IN in_type int, IN in_title varchar(32), IN in_content varchar(32), IN in_acctime int, IN in_deltime int, IN in_isread tinyint, IN in_isdel tinyint, IN in_itemsn1 int, IN in_itemnum1 int, IN in_itemsn2 int, IN in_itemnum2 int, IN in_itemsn3 int, IN in_itemnum3 int, IN in_itemsn4 int, IN in_itemnum4 int, IN in_itemsn5 int, IN in_itemnum5 int, IN in_isreward tinyint)
BEGIN 
...
DELIMITER ;
DELIMITER $$
CREATE DEFINER=`root`@`%` PROCEDURE `qy_insert_u_new_friend`(IN in_id bigint, IN in_self_csv_id int, IN in_friend_csv_id int, IN in_isdelete tinyint, IN in_recvtime int, IN in_heartamount int, IN in_update_time int, IN in_ifrecved tinyint)
BEGIN 
insert into u_new_friend (`id`, `self_csv_id`, `friend_csv_id`, `isdelete`, `recvtime`, `heartamount`, `update_time`, `ifrecved`) values (in_id, in_self_csv_id, in_friend_csv_id, in_isdelete, in_recvtime, in_heartamount, in_update_time, in_ifrecved)
on duplicate key update `id` = in_id, `self_csv_id` = in_self_csv_id, `friend_csv_id` = in_friend_csv_id, `isdelete` = in_isdelete, `recvtime` = in_recvtime, `heartamount` = in_heartamount, `update_time` = in_update_time, `ifrecved` = in_ifrecved;
END$$ 
DELIMITER ;
DELIMITER $$
...
DELIMITER $$
<<<<<<< HEAD
CREATE DEFINER=`root`@`%` PROCEDURE `qy_insert_users`(IN in_csv_id int, IN in_uname varchar(32), IN in_uviplevel int, IN in_config_sound tinyint, IN in_config_music tinyint, IN in_avatar int, IN in_sign varchar(255), IN in_c_role_id int, IN in_ifonline tinyint, IN in_level int, IN in_combat int, IN in_defense int, IN in_critical_hit int, IN in_blessing int, IN in_permission tinyint, IN in_modify_uname_count tinyint, IN in_onlinetime int, IN in_iconid int, IN in_is_valid tinyint, IN in_recharge_rmb int, IN in_recharge_diamond int, IN in_uvip_progress int, IN in_checkin_num int, IN in_checkin_reward_num int, IN in_exercise_level int, IN in_cgold_level int, IN in_gold_max int, IN in_exp_max int, IN in_equipment_enhance_success_rate_up_p int, IN in_store_refresh_count_max int, IN in_prop_refresh int, IN in_arena_frozen_time int, IN in_purchase_hp_count int, IN in_gain_gold_up_p int, IN in_gain_exp_up_p int, IN in_purchase_hp_count_max int, IN in_SCHOOL_reset_count_max int, IN in_SCHOOL_reset_count int, IN in_signup_time int, IN in_pemail_csv_id int, IN in_take_diamonds int, IN in_draw_number int, IN in_ifxilian int, IN in_cp_chapter int, IN in_cp_hanging_id int, IN in_cp_battle_id int, IN in_cp_battle_chapter int, IN in_lilian_level int, IN in_lilian_exp int, IN in_lilian_phy_power int, IN in_purch_lilian_phy_power int, IN in_ara_role_id1 int, IN in_ara_role_id2 int, IN in_ara_role_id3 int, IN in_ara_win_tms int, IN in_ara_lose_tms int, IN in_ara_tie_tms int, IN in_ara_clg_tms int, IN in_ara_clg_tms_pur_tms int, IN in_ara_integral int, IN in_ara_fighting tinyint, IN in_ara_interface tinyint, IN in_ara_rfh_cost_tms int, IN in_ara_clg_cost_tms int, IN in_sum_combat int, IN in_sum_defense int, IN in_sum_critical_hit int, IN in_sum_king int)
BEGIN 
insert into users (`csv_id`, `uname`, `uviplevel`, `config_sound`, `config_music`, `avatar`, `sign`, `c_role_id`, `ifonline`, `level`, `combat`, `defense`, `critical_hit`, `blessing`, `permission`, `modify_uname_count`, `onlinetime`, `iconid`, `is_valid`, `recharge_rmb`, `recharge_diamond`, `uvip_progress`, `checkin_num`, `checkin_reward_num`, `exercise_level`, `cgold_level`, `gold_max`, `exp_max`, `equipment_enhance_success_rate_up_p`, `store_refresh_count_max`, `prop_refresh`, `arena_frozen_time`, `purchase_hp_count`, `gain_gold_up_p`, `gain_exp_up_p`, `purchase_hp_count_max`, `SCHOOL_reset_count_max`, `SCHOOL_reset_count`, `signup_time`, `pemail_csv_id`, `take_diamonds`, `draw_number`, `ifxilian`, `cp_chapter`, `cp_hanging_id`, `cp_battle_id`, `cp_battle_chapter`, `lilian_level`, `lilian_exp`, `lilian_phy_power`, `purch_lilian_phy_power`, `ara_role_id1`, `ara_role_id2`, `ara_role_id3`, `ara_win_tms`, `ara_lose_tms`, `ara_tie_tms`, `ara_clg_tms`, `ara_clg_tms_pur_tms`, `ara_integral`, `ara_fighting`, `ara_interface`, `ara_rfh_cost_tms`, `ara_clg_cost_tms`, `sum_combat`, `sum_defense`, `sum_critical_hit`, `sum_king`) values (in_csv_id, in_uname, in_uviplevel, in_config_sound, in_config_music, in_avatar, in_sign, in_c_role_id, in_ifonline, in_level, in_combat, in_defense, in_critical_hit, in_blessing, in_permission, in_modify_uname_count, in_onlinetime, in_iconid, in_is_valid, in_recharge_rmb, in_recharge_diamond, in_uvip_progress, in_checkin_num, in_checkin_reward_num, in_exercise_level, in_cgold_level, in_gold_max, in_exp_max, in_equipment_enhance_success_rate_up_p, in_store_refresh_count_max, in_prop_refresh, in_arena_frozen_time, in_purchase_hp_count, in_gain_gold_up_p, in_gain_exp_up_p, in_purchase_hp_count_max, in_SCHOOL_reset_count_max, in_SCHOOL_reset_count, in_signup_time, in_pemail_csv_id, in_take_diamonds, in_draw_number, in_ifxilian, in_cp_chapter, in_cp_hanging_id, in_cp_battle_id, in_cp_battle_chapter, in_lilian_level, in_lilian_exp, in_lilian_phy_power, in_purch_lilian_phy_power, in_ara_role_id1, in_ara_role_id2, in_ara_role_id3, in_ara_win_tms, in_ara_lose_tms, in_ara_tie_tms, in_ara_clg_tms, in_ara_clg_tms_pur_tms, in_ara_integral, in_ara_fighting, in_ara_interface, in_ara_rfh_cost_tms, in_ara_clg_cost_tms, in_sum_combat, in_sum_defense, in_sum_critical_hit, in_sum_king)
on duplicate key update `csv_id` = in_csv_id, `uname` = in_uname, `uviplevel` = in_uviplevel, `config_sound` = in_config_sound, `config_music` = in_config_music, `avatar` = in_avatar, `sign` = in_sign, `c_role_id` = in_c_role_id, `ifonline` = in_ifonline, `level` = in_level, `combat` = in_combat, `defense` = in_defense, `critical_hit` = in_critical_hit, `blessing` = in_blessing, `permission` = in_permission, `modify_uname_count` = in_modify_uname_count, `onlinetime` = in_onlinetime, `iconid` = in_iconid, `is_valid` = in_is_valid, `recharge_rmb` = in_recharge_rmb, `recharge_diamond` = in_recharge_diamond, `uvip_progress` = in_uvip_progress, `checkin_num` = in_checkin_num, `checkin_reward_num` = in_checkin_reward_num, `exercise_level` = in_exercise_level, `cgold_level` = in_cgold_level, `gold_max` = in_gold_max, `exp_max` = in_exp_max, `equipment_enhance_success_rate_up_p` = in_equipment_enhance_success_rate_up_p, `store_refresh_count_max` = in_store_refresh_count_max, `prop_refresh` = in_prop_refresh, `arena_frozen_time` = in_arena_frozen_time, `purchase_hp_count` = in_purchase_hp_count, `gain_gold_up_p` = in_gain_gold_up_p, `gain_exp_up_p` = in_gain_exp_up_p, `purchase_hp_count_max` = in_purchase_hp_count_max, `SCHOOL_reset_count_max` = in_SCHOOL_reset_count_max, `SCHOOL_reset_count` = in_SCHOOL_reset_count, `signup_time` = in_signup_time, `pemail_csv_id` = in_pemail_csv_id, `take_diamonds` = in_take_diamonds, `draw_number` = in_draw_number, `ifxilian` = in_ifxilian, `cp_chapter` = in_cp_chapter, `cp_hanging_id` = in_cp_hanging_id, `cp_battle_id` = in_cp_battle_id, `cp_battle_chapter` = in_cp_battle_chapter, `lilian_level` = in_lilian_level, `lilian_exp` = in_lilian_exp, `lilian_phy_power` = in_lilian_phy_power, `purch_lilian_phy_power` = in_purch_lilian_phy_power, `ara_role_id1` = in_ara_role_id1, `ara_role_id2` = in_ara_role_id2, `ara_role_id3` = in_ara_role_id3, `ara_win_tms` = in_ara_win_tms, `ara_lose_tms` = in_ara_lose_tms, `ara_tie_tms` = in_ara_tie_tms, `ara_clg_tms` = in_ara_clg_tms, `ara_clg_tms_pur_tms` = in_ara_clg_tms_pur_tms, `ara_integral` = in_ara_integral, `ara_fighting` = in_ara_fighting, `ara_interface` = in_ara_interface, `ara_rfh_cost_tms` = in_ara_rfh_cost_tms, `ara_clg_cost_tms` = in_ara_clg_cost_tms, `sum_combat` = in_sum_combat, `sum_defense` = in_sum_defense, `sum_critical_hit` = in_sum_critical_hit, `sum_king` = in_sum_king;
=======
CREATE DEFINER=`root`@`%` PROCEDURE `qy_insert_users`(IN in_csv_id int, IN in_uname varchar(32), IN in_uviplevel int, IN in_config_sound tinyint, IN in_config_music tinyint, IN in_avatar int, IN in_sign varchar(255), IN in_c_role_id int, IN in_ifonline tinyint, IN in_level int, IN in_combat int, IN in_defense int, IN in_critical_hit int, IN in_blessing int, IN in_permission tinyint, IN in_modify_uname_count tinyint, IN in_onlinetime int, IN in_iconid int, IN in_is_valid tinyint, IN in_recharge_rmb int, IN in_recharge_diamond int, IN in_uvip_progress int, IN in_checkin_num int, IN in_checkin_reward_num int, IN in_exercise_level int, IN in_cgold_level int, IN in_gold_max int, IN in_exp_max int, IN in_equipment_enhance_success_rate_up_p int, IN in_store_refresh_count_max int, IN in_prop_refresh int, IN in_arena_frozen_time int, IN in_purchase_hp_count int, IN in_gain_gold_up_p int, IN in_gain_exp_up_p int, IN in_purchase_hp_count_max int, IN in_SCHOOL_reset_count_max int, IN in_SCHOOL_reset_count int, IN in_signup_time int, IN in_pemail_csv_id int, IN in_take_diamonds int, IN in_draw_number int, IN in_ifxilian int, IN in_cp_chapter int, IN in_cp_hanging_id int, IN in_cp_battle_id int, IN in_cp_battle_chapter int, IN in_lilian_level int, IN in_lilian_exp int, IN in_lilian_phy_power int, IN in_purch_lilian_phy_power int, IN in_ara_role_id1 int, IN in_ara_role_id2 int, IN in_ara_role_id3 int, IN in_ara_win_tms int, IN in_ara_lose_tms int, IN in_ara_tie_tms int, IN in_ara_clg_tms int, IN in_ara_clg_tms_pur_tms int, IN in_ara_integral int, IN in_ara_fighting tinyint, IN in_ara_interface tinyint, IN in_ara_rfh_cost_tms int, IN in_ara_clg_cost_tms int)
BEGIN 
insert into users (`csv_id`, `uname`, `uviplevel`, `config_sound`, `config_music`, `avatar`, `sign`, `c_role_id`, `ifonline`, `level`, `combat`, `defense`, `critical_hit`, `blessing`, `permission`, `modify_uname_count`, `onlinetime`, `iconid`, `is_valid`, `recharge_rmb`, `recharge_diamond`, `uvip_progress`, `checkin_num`, `checkin_reward_num`, `exercise_level`, `cgold_level`, `gold_max`, `exp_max`, `equipment_enhance_success_rate_up_p`, `store_refresh_count_max`, `prop_refresh`, `arena_frozen_time`, `purchase_hp_count`, `gain_gold_up_p`, `gain_exp_up_p`, `purchase_hp_count_max`, `SCHOOL_reset_count_max`, `SCHOOL_reset_count`, `signup_time`, `pemail_csv_id`, `take_diamonds`, `draw_number`, `ifxilian`, `cp_chapter`, `cp_hanging_id`, `cp_battle_id`, `cp_battle_chapter`, `lilian_level`, `lilian_exp`, `lilian_phy_power`, `purch_lilian_phy_power`, `ara_role_id1`, `ara_role_id2`, `ara_role_id3`, `ara_win_tms`, `ara_lose_tms`, `ara_tie_tms`, `ara_clg_tms`, `ara_clg_tms_pur_tms`, `ara_integral`, `ara_fighting`, `ara_interface`, `ara_rfh_cost_tms`, `ara_clg_cost_tms`) values (in_csv_id, in_uname, in_uviplevel, in_config_sound, in_config_music, in_avatar, in_sign, in_c_role_id, in_ifonline, in_level, in_combat, in_defense, in_critical_hit, in_blessing, in_permission, in_modify_uname_count, in_onlinetime, in_iconid, in_is_valid, in_recharge_rmb, in_recharge_diamond, in_uvip_progress, in_checkin_num, in_checkin_reward_num, in_exercise_level, in_cgold_level, in_gold_max, in_exp_max, in_equipment_enhance_success_rate_up_p, in_store_refresh_count_max, in_prop_refresh, in_arena_frozen_time, in_purchase_hp_count, in_gain_gold_up_p, in_gain_exp_up_p, in_purchase_hp_count_max, in_SCHOOL_reset_count_max, in_SCHOOL_reset_count, in_signup_time, in_pemail_csv_id, in_take_diamonds, in_draw_number, in_ifxilian, in_cp_chapter, in_cp_hanging_id, in_cp_battle_id, in_cp_battle_chapter, in_lilian_level, in_lilian_exp, in_lilian_phy_power, in_purch_lilian_phy_power, in_ara_role_id1, in_ara_role_id2, in_ara_role_id3, in_ara_win_tms, in_ara_lose_tms, in_ara_tie_tms, in_ara_clg_tms, in_ara_clg_tms_pur_tms, in_ara_integral, in_ara_fighting, in_ara_interface, in_ara_rfh_cost_tms, in_ara_clg_cost_tms)
on duplicate key update `csv_id` = in_csv_id, `uname` = in_uname, `uviplevel` = in_uviplevel, `config_sound` = in_config_sound, `config_music` = in_config_music, `avatar` = in_avatar, `sign` = in_sign, `c_role_id` = in_c_role_id, `ifonline` = in_ifonline, `level` = in_level, `combat` = in_combat, `defense` = in_defense, `critical_hit` = in_critical_hit, `blessing` = in_blessing, `permission` = in_permission, `modify_uname_count` = in_modify_uname_count, `onlinetime` = in_onlinetime, `iconid` = in_iconid, `is_valid` = in_is_valid, `recharge_rmb` = in_recharge_rmb, `recharge_diamond` = in_recharge_diamond, `uvip_progress` = in_uvip_progress, `checkin_num` = in_checkin_num, `checkin_reward_num` = in_checkin_reward_num, `exercise_level` = in_exercise_level, `cgold_level` = in_cgold_level, `gold_max` = in_gold_max, `exp_max` = in_exp_max, `equipment_enhance_success_rate_up_p` = in_equipment_enhance_success_rate_up_p, `store_refresh_count_max` = in_store_refresh_count_max, `prop_refresh` = in_prop_refresh, `arena_frozen_time` = in_arena_frozen_time, `purchase_hp_count` = in_purchase_hp_count, `gain_gold_up_p` = in_gain_gold_up_p, `gain_exp_up_p` = in_gain_exp_up_p, `purchase_hp_count_max` = in_purchase_hp_count_max, `SCHOOL_reset_count_max` = in_SCHOOL_reset_count_max, `SCHOOL_reset_count` = in_SCHOOL_reset_count, `signup_time` = in_signup_time, `pemail_csv_id` = in_pemail_csv_id, `take_diamonds` = in_take_diamonds, `draw_number` = in_draw_number, `ifxilian` = in_ifxilian, `cp_chapter` = in_cp_chapter, `cp_hanging_id` = in_cp_hanging_id, `cp_battle_id` = in_cp_battle_id, `cp_battle_chapter` = in_cp_battle_chapter, `lilian_level` = in_lilian_level, `lilian_exp` = in_lilian_exp, `lilian_phy_power` = in_lilian_phy_power, `purch_lilian_phy_power` = in_purch_lilian_phy_power, `ara_role_id1` = in_ara_role_id1, `ara_role_id2` = in_ara_role_id2, `ara_role_id3` = in_ara_role_id3, `ara_win_tms` = in_ara_win_tms, `ara_lose_tms` = in_ara_lose_tms, `ara_tie_tms` = in_ara_tie_tms, `ara_clg_tms` = in_ara_clg_tms, `ara_clg_tms_pur_tms` = in_ara_clg_tms_pur_tms, `ara_integral` = in_ara_integral, `ara_fighting` = in_ara_fighting, `ara_interface` = in_ara_interface, `ara_rfh_cost_tms` = in_ara_rfh_cost_tms, `ara_clg_cost_tms` = in_ara_clg_cost_tms;
>>>>>>> 22dfcf9ad796237021cd0f93acf2e24e5468ba2d
END$$ 
DELIMITER ;
DELIMITER $$
