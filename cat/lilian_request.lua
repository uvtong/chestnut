...
function REQUEST:get_lilian_info()
	...
	if r.phy_power >= user.phy_power then
		ret.phy_power = r.phy_power
	elseif r.phy_power > user.phy_power then
		

	end 

	ret.phy_power = user.lilian_power

	if user.lilian_power < r.phy_power then
		skynet.timeout( )
	end 
	...
	for k , v in ipairs( user.u_lilianmgr.__data ) do
		...
		tmp.id = v.quanguan_id
		tmp.lilian_num = 
		
		if date >= v.end_time then
			tmp.left_cd_time = 0 
		else
			tmp.left.cd_time = v.end_time - date
