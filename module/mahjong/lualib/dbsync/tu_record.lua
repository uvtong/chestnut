...
local tname = "tu_record"

local _M = {}

function _M.cache_select(db, uid, ... )
	-- body
	local sql = string.format("select * from tu_record where uid = %s", uid)
	local res = query.select(tname, sql)
	if #rse > 0 then
		for k,v in pairs(res) do
			local id = v['id']
			db:zadd(string.format("tu_record:%d", uid), 1, id)

			for kk,vv in pairs(v) do
				db:hset(string.format("tu_record:%d:%d", uid, id), kk, vv)
			end
		end
	else
	end
