...
function _M.set_row_cache(t, pk, value)
	-- body
...
function _M.get_row_cache(t, pk)
	-- body
	assert(type(pk) == t.__head[t.__pk].t)
	local k
	if t.__head[t.__pk].t == "string" then
...
-- this function 
function _M.load_db_to_cache(t, key, value)
	-- body
	local sql
...
function _M.load_db(t, key, value)
	-- body
	local sql
	if key ~= nil then
		if key == "pk" then
...
function _M.load_cache(t, key, value)
	-- body
...
function _M.load_cache_to_sd(t)
	-- body
end

function _M.load_cache_to_stm(t, ... )
	-- body
end

function _M.load_data_to_stm(t, ctx, child)
	-- body
	if t.__stm then
		-- ctx.__data
		local r = {}
...
function _M.load_stm_to_data(ctx, child)
	-- body
	if t.__stm then
	end
end
